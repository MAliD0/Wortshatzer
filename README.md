...
- optional closest-word suggestion URL template, selectors, and fallback selectors

The HTTP lookup layer reuses one client, rejects oversized pages, converts network/status errors into user-safe dictionary errors, and caches successful results for 12 hours. If a response is a spellcheck page—whether opened directly or reached by redirect—suggestion recovery runs before normal entry extraction. The entry selector is applied only after a suggestion link resolves to a non-spellcheck dictionary page, so unrelated widgets and missing entry containers on the suggestion page cannot break or contaminate extraction. Otherwise, recovery explicitly requests the profile's configured suggestion URL after normal extraction fails. The Cambridge profile uses `https://dictionary.cambridge.org/spellcheck/german-english/?q={word}`. The scraper reads candidate links from that page, ranks them by edit distance from the entered word, follows the closest same-host HTTPS link once, and retries normal extraction. The built-in Cambridge German–English profile includes this stage. Cambridge and Verbformen profiles remain editable starting points because websites can change.

Open **Dictionary settings** from the main window to create or clone a profile. Enable **Use closest-word fallback**, provide its suggestion URL template with `{word}`, and configure its primary and fallback CSS selectors when a dictionary has a spelling-suggestion page. Built-ins cannot be overwritten or deleted. Custom profiles are stored atomically under `%LOCALAPPDATA%\\Wortshatzer\\scraper-profiles.json`, and **Test profile** previews the current unsaved selectors against a real word. Cache keys include the complete profile configuration, so changing a selector always produces a fresh preview.
