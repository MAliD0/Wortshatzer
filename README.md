...
The HTTP lookup layer reuses one client, rejects oversized pages, converts network/status errors into user-safe dictionary errors, and caches successful results for 12 hours. If a request is redirected to a spellcheck page, suggestion recovery runs before normal entry extraction so unrelated dictionary widgets cannot be mistaken for the requested word. Otherwise, recovery explicitly requests the profile's configured suggestion URL after normal extraction fails. The Cambridge profile uses `https://dictionary.cambridge.org/spellcheck/german-english/?q={word}`. The scraper reads candidate links from that page, ranks them by edit distance from the entered word, follows the closest same-host HTTPS link once, and retries normal extraction. The built-in Cambridge German–English profile includes this stage. Cambridge and Verbformen profiles remain editable starting points because websites can change.
